...
| `.../customTable/plugins/fieldRender/index.ts` | 不增加条码生成或特殊 renderer；设计态仍按普通字段展示字段名 | 拖入字段、切换展示形式、选区刷新、合并单元格 |
| `.../customTable/utils/cellData.ts` | 不增加条码渲染状态，只确认 `otherShowType` / `encodeType` 不被属性更新覆盖 | 修改编码类型后 schema 正确 |
...
| 文件 | 计划改动 | 验证点 |
|---|---|---|
| `packages/@newgrand/udp-report-table/src/preview/plugins/imageRender/index.tsx` | 复用二维码/图片既有 renderer，直接消费后端返回的 `formatData` 图片地址或 Data URL | 接口返回图片、空值、合并单元格 |
| `.../preview/tableSheets/index.tsx` | 维持现有 `image` cell type 注册，不新增条形码专用 renderer/cell type | 多 Sheet 切换后仍正确渲染 |
| `.../preview/plugins/textRender.tsx` | 不增加 `bar_code` 分支，不在前端编码或生成 SVG/Canvas | 普通文本、链接、过滤渲染无回归 |

#### C. 条码生成方案决策（2026-07-28 联调修订，覆盖 2026-07-27 方案）

后端已明确：条形码由后端生成图片，前端只负责保存配置并在运行预览中展示图片。因此取消前端生成方案，不引入 `JsBarcode`、`bwip-js` 或其他条码依赖。

| 方案 | 结论 | 原因 |
|---|---|---|
| 后端生成图片 + 复用二维码图片 renderer | **采用** | 与现有二维码链路一致；无新增前端依赖；前端不重复实现码制和校验 |
| `JsBarcode` / `bwip-js` | 取消 | 后端已经生成最终图片，前端再次编码会造成规则重复、包体增加和结果不一致 |
| DOM/Canvas/SVG 前端生成 | 取消 | 不属于前端职责，且会产生预览与后端打印结果不一致的风险 |

渲染策略：

1. **设计态**：展示普通字段名，只提供“条形码”和编码类型配置，不生成条码示意。
2. **运行预览**：后端将条形码结果按二维码现有契约返回为图片单元格；前端 `imageRender` 使用 `formatData` 展示。
3. **码制与校验**：`code_128`、`code_39`、`ean_13` 的生成和非法值处理完全由后端负责。
4. **默认编码**：用户首次切换到条形码时写入 `code_128`；切换离开时保留 `encodeType`，便于切回。
5. **依赖边界**：`udp-report-table` 和 `report-web` 均不增加条码生成依赖，不导出条码生成 API。

#### D. `report-web`：打印/PDF

| 文件 | 计划改动 | 验证点 |
|---|---|---|
| `src/components/report/print/utils/PDF.ts` | 本次不增加前端条码生成逻辑；若后端打印数据同样按图片契约返回，则复用现有图片处理 | 普通图片、二维码、条形码图片回归 |
| `src/pages/TableManager/preview/index.tsx` | 不新增业务逻辑，由 `ReportTable` 的既有图片 renderer 展示后端结果 | 预览接口返回条形码图片后正常显示 |
...
- [ ] 设计态保持普通字段展示，不在前端生成条码。
- [ ] 后端返回条形码图片后，运行预览复用二维码/图片 renderer 正确显示并保持比例。
- [ ] 切换普通文本、二维码、条形码时不残留错误渲染状态；前端不新增条码生成依赖。
- [ ] 空值、超长值和非法值由后端按图片生成协议处理，不阻断前端页面。
...
- 联调口径修订：条形码由后端生成图片，前端只保存 `otherShowType=bar_code` 与 `encodeType`，运行预览复用二维码/图片 renderer。
- 取消 `JsBarcode` 依赖、前端 SVG 生成器、设计态条码占位和文本 renderer 条码分支；`report-web` 不新增条码生成逻辑。
- 保留“条形码”展示类型、三种后端编码枚举和首次选择时的 `code_128` 默认值。
- `udp-report-table` 构建通过，架构回归与现有测试共 54 项全部通过；本地测试包已同步，等待页面联调确认。
- 当前代码尚未形成新的修订提交，版本变更记录的 Commit 暂记为待关联；完成页面验收并提交后再更新版本覆盖矩阵。
