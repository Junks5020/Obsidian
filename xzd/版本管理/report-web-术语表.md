...
**主字段 (Primary Field)**:
单元格绑定的数据集字段；二维码内容拼接时固定排在第一行，不进入追加展示字段列表。
_Avoid_: 绑定字段、第一字段

**追加展示字段 (QR Extra Field)**:
二维码展示形式下，由用户从单元格所属数据集中追加选择的附加字段；每项可独立配置格式化，其排列顺序决定二维码内容的拼接顺序。空值的追加展示字段不跳过，仍展示字段名。
_Avoid_: 附加字段、扩展字段、二维码字段
