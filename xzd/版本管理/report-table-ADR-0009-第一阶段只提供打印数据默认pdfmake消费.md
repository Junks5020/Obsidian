...
独立 `pdfmake` 示例曾作为第一阶段接入参考交付，但因其不进入运行时和发布包、无仓库内消费者且仅提供极简排版示范，现已从代码库移除。组件职责收敛为提供打印数据类型、校验/序列化能力和消费边界说明；本 ADR 中关于 `pdfmake` 由业务应用消费、组件不负责排版与打印流程的约束保持不变。详见 [[report-table-打印示例删除决策-20260825]]。
