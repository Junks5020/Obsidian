...
**附件分类**：现代附件按业务来源划分的五个类别——单据附件、审批前附件、审批后附件、来源单据附件、工作流附件。审批前附件是单据附件在单据已审批后的只读呈现，二者共用同一份权限。
...
**附件可见性**：当前激活分类权限块中的 `visible` 开关；当其为禁止或权限初始化失败时，现代附件区域整体不显示。分类树模式未选中类型时，任一类型可见即保持区域可见。
