...
“运行时中文候选”包括字符串字面量、模板字符串和 JSX 文本，可作为国际化抽离的工作清单。下表已按 [[ng-design-ADR-0003-Attachment中文消息具名占位符协议|simple-v1]] 将动态表达式转换为 `{{name}}` 具名占位符；静态文案保持原样。日志、异常消息和接口转换文本也包含在内，是否面向最终用户需要在改造时结合调用链确认。注释、JSDoc 和样式注释不属于运行时文案，文末审计附录继续保留当前源码原文。

## 多语言消息表

本表共 57 条，覆盖 Attachment 中需要进入语言包的用户可见文案及可能透传给用户的错误消息。动态内容统一采用 [[ng-design-ADR-0003-Attachment中文消息具名占位符协议|simple-v1]] 的 `{{name}}` 具名占位符。

| Message key | 中文模板（simple-v1） | 变量 | 使用场景 | 源码位置 |
| --- | --- | --- | --- | --- |
| `attachment.action.cancel` | 取消 | - | 关闭附件来源选择 | `attachment-upload.tsx:336`<br>`AttachOffline/upload.tsx:250`<br>`upload/upload.tsx:590` |
| `attachment.action.upload` | 上传附件 | - | 上传入口 | `attachment-upload.tsx:257`<br>`upload/upload.tsx:488` |
| `attachment.action.uploadOffline` | 上传附件(离线) | - | 离线上传入口 | `AttachOffline/upload.tsx:180` |
| `attachment.action.viewDocument` | 查看文档 | - | 文档预览入口 | `custom-preview.tsx:91` |
| `attachment.comment.sensitive` | 办理意见中包含敏感字:{{words}}{{suffix}},请修改后重新提交! | `words`, `suffix` | 敏感词校验提示 | `util.ts:92` |
| `attachment.common.label` | 附件 | - | 附件组件默认标签 | `attachment-upload.tsx:26`<br>`AttachOffline/upload.tsx:31`<br>`upload/upload.tsx:38` |
| `attachment.delete.failed` | 删除失败 | - | 删除接口失败提示 | `api/attachemntDelete/response.ts:21` |
| `attachment.delete.success` | 删除成功 | - | 离线附件删除成功 | `AttachOffline/index.tsx:71` |
| `attachment.info.loadFailed` | 附件信息获取失败 | - | 附件详情加载失败 | `AttachmentView/index.ts:144` |
| `attachment.init.failed` | 附件初始化失败 | - | 附件初始化失败 | `attachment.tsx:293` |
| `attachment.init.missingTable` | 单据附件未关联表名！ | - | 初始化参数校验 | `attachment.tsx:87` |
| `attachment.init.offlineUnsupported` | 离线状态不能初始化 | - | 离线初始化限制 | `attachment.tsx:376` |
| `attachment.meta.beforeUploadTime` | 上传前修改时间: | - | 附件元数据标签 | `AttachOffline/Card.tsx:14`<br>`Card.tsx:47` |
| `attachment.meta.uploader` | 上传人: | - | 附件元数据标签 | `AttachOffline/Card.tsx:50`<br>`Card.tsx:76` |
| `attachment.meta.uploadTime` | 上传时间: | - | 附件元数据标签 | `Card.tsx:77` |
| `attachment.offline.defaultItem` | 离线数据{{index}} | `index` | 离线数据默认名称 | `AttachOffline/OfflineListHelp.tsx:48` |
| `attachment.offline.emptyDeletePath` | 文件路径为空，无法删除 | - | 离线附件删除校验 | `AttachOffline/index.tsx:64` |
| `attachment.offline.emptyPreviewPath` | 文件路径为空，无法预览 | - | 离线附件预览校验 | `AttachOffline/index.tsx:44` |
| `attachment.offline.filter` | 筛选 | - | 离线数据筛选标题 | `AttachOffline/OfflineListHelp.tsx:28` |
| `attachment.offline.noMatch` | 无匹配选项 | - | 离线筛选空结果 | `AttachOffline/OfflineListHelp.tsx:179` |
| `attachment.offline.noMore` | 没有更多了 | - | 离线筛选列表底部 | `AttachOffline/OfflineListHelp.tsx:181` |
| `attachment.offline.previewRetry` | 预览失败，请重试 | - | 离线附件预览失败 | `AttachOffline/index.tsx:54` |
| `attachment.offline.reference` | 引用离线数据 | - | 离线数据引用入口 | `AttachOffline/OfflineListHelp.tsx:99` |
| `attachment.offline.search` | 搜索 | - | 离线数据搜索占位符 | `AttachOffline/OfflineListHelp.tsx:148` |
| `attachment.preview.firstPage` | 已是第一页 | - | PDF 翻页边界提示 | `attachment-preview.tsx:117` |
| `attachment.preview.lastPage` | 已是最后一页 | - | PDF 翻页边界提示 | `attachment-preview.tsx:131` |
| `attachment.preview.loadFailed` | 附件加载失败 | - | 预览资源加载失败 | `attachment-preview.tsx:179` |
| `attachment.preview.notFound` | 附件不存在 | - | 预览资源不存在 | `util.ts:220` |
| `attachment.preview.unsupported` | 当前文件格式不支持预览 | - | 不支持的预览格式 | `attachment-preview.tsx:271`<br>`attachment-preview.tsx:313`<br>`attachment-preview.tsx:355` |
| `attachment.preview.urlFailed` | 附件预览地址获取失败 | - | 预览地址请求失败 | `AttachmentView/index.ts:106` |
| `attachment.preview.urlMissing` | 附件预览地址不存在 | - | 预览地址为空 | `AttachmentView/index.ts:156`<br>`util.ts:242` |
| `attachment.source.album` | 相册 | - | 附件来源 | `accept.tsx:35`<br>`upload/accept.tsx:36` |
| `attachment.source.burst` | 连拍 | - | 附件来源 | `accept.tsx:28`<br>`upload/accept.tsx:22` |
| `attachment.source.file` | 文件 | - | 附件来源 | `accept.tsx:49`<br>`upload/accept.tsx:57` |
| `attachment.source.photo` | 拍照 | - | 附件来源 | `accept.tsx:14`<br>`accept.tsx:21`<br>`upload/accept.tsx:15`<br>`upload/accept.tsx:28` |
| `attachment.source.prompt` | 请选择来源 | - | 附件来源弹层标题 | `attachment-upload.tsx:332`<br>`AttachOffline/upload.tsx:229`<br>`upload/upload.tsx:571` |
| `attachment.source.signature` | 签名 | - | 附件来源 | `accept.tsx:59`<br>`upload/accept.tsx:50` |
| `attachment.source.video` | 录像 | - | 附件来源 | `accept.tsx:42`<br>`upload/accept.tsx:43` |
| `attachment.type.all` | 全部 | - | 附件分类全部选项 | `attachment-type-list.tsx:64` |
| `attachment.upload.allowedFormats` | 仅支持上传：{{formats}} 格式 | `formats` | 文件格式白名单校验 | `upload/validateAttachOptions.ts:29` |
| `attachment.upload.blockedFormats` | 不支持上传：{{formats}} 格式 | `formats` | 文件格式黑名单校验 | `upload/validateAttachOptions.ts:39` |
| `attachment.upload.doneCount` | 已上传{{count}}文件 | `count` | 上传完成数量 | `attachment-upload.tsx:350` |
| `attachment.upload.failed` | 上传失败 | - | 上传异常或接口失败 | `api/attachemntUpload/customizedUploadResponse.ts:25`<br>`upload/api/uploadFileNew/response.ts:28`<br>`upload/upload.tsx:130`<br>`upload/upload.tsx:202`<br>`upload/upload.tsx:246`<br>`upload/upload.tsx:300`<br>`upload/upload.tsx:332`<br>`upload/upload.tsx:367`<br>`upload/util.ts:145`<br>`upload/util.ts:175`<br>`upload/util.ts:180` |
| `attachment.upload.invalidFileName` | 文件名格式不符合要求 | - | 文件名格式校验 | `upload/validateAttachOptions.ts:66` |
| `attachment.upload.limitReached` | 附件上传数量已达上限 | - | 上传入口禁用说明 | `attachment-upload.tsx:253`<br>`AttachOffline/upload.tsx:176`<br>`upload/upload.tsx:484` |
| `attachment.upload.locationFailed` | 获取位置失败 | - | 水印定位失败 | `upload/upload.tsx:162` |
| `attachment.upload.maxCount` | 最大上传数 {{maxCount}} | `maxCount` | 上传数量上限提示 | `attachment-upload.tsx:291`<br>`attachment-upload.tsx:312`<br>`AttachOffline/upload.tsx:208`<br>`upload/upload.tsx:519`<br>`upload/upload.tsx:545` |
| `attachment.upload.maxSize` | 文件大小不能超过 {{maxSize}}MB! | `maxSize` | 文件大小强校验 | `upload/validateAttachOptions.ts:50` |
| `attachment.upload.noPermission` | 暂无上传权限 | - | 上传入口禁用说明 | `attachment-upload.tsx:246`<br>`AttachOffline/upload.tsx:169`<br>`upload/upload.tsx:477` |
| `attachment.upload.sensitiveFileName` | 文件名包含敏感词，请修改后重试 | - | 文件名敏感词校验 | `upload/validateAttachOptions.ts:77` |
| `attachment.upload.status.failed` | 失败 | - | 上传进度状态 | `upload/upload.tsx:614` |
| `attachment.upload.status.initializing` | 上传初始化中 | - | 上传进度状态 | `upload/upload.tsx:626` |
| `attachment.upload.status.success` | 成功 | - | 上传进度状态 | `upload/upload.tsx:607` |
| `attachment.upload.status.total` | 合计 | - | 上传进度统计 | `upload/upload.tsx:621` |
| `attachment.upload.status.uploading` | 正在上传 | - | 上传进度标题 | `attachment-upload.tsx:346`<br>`upload/upload.tsx:600` |
| `attachment.upload.totalCount` | 共{{count}}文件 | `count` | 上传文件总数 | `attachment-upload.tsx:351` |
| `attachment.upload.warnSize` | 文件大小已超过 {{warnSize}}MB, 是否继续上传! | `warnSize` | 文件大小提醒 | `upload/validateAttachOptions.ts:56` |

以下 6 条不进入多语言表：`离线数据查询返回空`、`离线数据返回类型异常:`、`离线数据解析失败`、`离线数据解析失败: query` 属于调试日志；`文件预览失败：` 是控制台错误前缀；`上传前修改时间`（`Card.tsx:45`）是既有配置匹配常量，翻译后会改变业务判断。
## 完整中文命中
